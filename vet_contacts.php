...
$stmt = $pdo->prepare('SELECT * FROM vet_contacts ORDER BY contact LIMIT :current_page, :record_per_page');
$stmt->bindValue(':current_page', ($page-1)*$records_per_page, PDO::PARAM_INT);
$stmt->bindValue(':record_per_page', $records_per_page, PDO::PARAM_INT);
$stmt->execute();
// Fetch the records so we can display them in our template.
$vet_contacts = $stmt->fetchAll(PDO::FETCH_ASSOC);

// Get the total number of breeds, this is so we can determine whether there should be a next and previous button
$num_microchip_details = $pdo->query('SELECT COUNT(*) FROM vet_contacts')->fetchColumn();
?>

<?=template_header('vet_contacts')?>
vet_contacts
